...
Inventory_Rectangle.prototype.set_perp_wh_vals = function(wh_array){

	this.perp_h = wh_array[0];
	this.perp_h = wh_array[1];
...
Inventory_Rectangle.prototype.set_initial_offsets = function(offset_x,offset_y){
	...
	//console.log("so...come here often?");
	...
	for(var i = 0; i < this.item_array.length; i++){
		...
		this.item_array[i] = tmp_box;

		// pw.print("tmp_box.x is: " + this.x);
		// pw.print("tmp_box.y is: " + this.y);
	}
...
Inventory_Rectangle.prototype.set_offsets = function(offset_array){
	...
	for(var i = 0; i < this.item_array.length; i++){
		...
		//console.log("how about here? getting in here?");
		//var tmp_box = this.item_array[i];
		this.item_array[i].set_offsets(offset_array[0],offset_array[1]);

		// pw.print("tmp_box.x is: " + this.item_array[i].x);
		// pw.print("tmp_box.y is: " + this.item_array[i].y);		
...
//reappear around player's feet. 
		...
		if(rect_contains_item){
			//rule 2
			//console.log("rule 2 or 3");
			if(this.one_or_more_boxes_containing_new_item_not_empty(item) && mlc == false){
				//console.log("rule 2");
			...
			//rule 4
			//console.log("checking for rule 4. if failed, see 'rule 1'");
			console.log("rule 4");
			if(this.box_containg_mouse_is_empty(mx,my)){
				//console.log("rule 4");
				this.update_rule_4(mx,my,mlc)
				return item;
			}
...
Inventory_Rectangle.prototype.box_containg_mouse_is_empty = function(item){
	...
	for(var i = 0; i < this.item_array.length; i++){
		...
		//see if that box contains the mouse 
		if(tmp_box.mouse_intersects(mx,my)){

			//pw.print("tmp_item.contains_item() is: " + tmp_box.contains_item());

			//and if it does, return whether or not the box contains an item. 
			return tmp_box.contains_item();
		}
	}
...
Inventory_Rectangle.prototype.set_perp_xy_vals = function(perp_xy_array){

	this.perp_x = perp_xy_array[0];
	this.perp_y = perp_xy_array[1];

};

Inventory_Rectangle.prototype.box_that_item_is_the_most_in_and_array_loc = function(item){
...
Inventory_Rectangle.prototype.update_rule_2 = function(item,mlc){
	...
	//console.log("update_rule_2 (got into rule 2!!!)");

	//array holding both the box and it's location in the array
	var tmp_array = this.box_that_item_is_the_most_in_and_array_loc(item);
	var tmp_box = tmp_array[0];
	var loc = tmp_array[1];

	//get box that item is the most in. then...
	//var tmp_box = this.box_item_is_the_most_in(item);

	// if(tmp_box != null){ console.log("(NOT NULL!) tmp_box.name is: " + tmp_box.name)} 
	// else{console.log("tmp_box was null");}
	...
	if(tmp_box != null){
		...
		//get the item in the box (clearing the box in the process)
		var tmp_item = tmp_box.get_item();

		//put the new item in the box 
		tmp_box.add_item(item,mlc);

		// var res = tmp_box.item == null ? "THE ITEM IS NOT THE BOX" : ("tmp_box.item.name is: " + tmp_box.item.name);
		// console.log(res);

		//put the box back into the array
		//this.item_array[loc] = tmp_box;

		//then set the item that is being taken out at the charter who owns that inventory's
		//feet. if it's a bookshelf or whatever, set it around that bookshelf. 
		//console.log("this.perp_h is: " + this.perp_h);
		tmp_item.set_at_owners_loc(this.perp_x,this.perp_y,this.perp_h);

		//a bug thing more than anything else. if not called, the item keeps getting dragged
		//around. 
		tmp_item.turn_off_dragging();

		//tmp_item.set_at_owners_loc(120,120);

		//then set it into gen_obj
		gen_obj = tmp_item;

		// if(gen_obj != null){console.log("(NOT NULL gen_obj.name is: " + gen_obj.name);}
		// else{console.log("gen_obj IS in fact null");}

		// var res = this.item_array[loc].item == null ? "THE ITEM IS NOT IN this.item_array[loc]" : (" this.item_array[loc].item.name is: " +  this.item_array[loc].item.name);
		// console.log(res);

		//console.log("this.item_array[loc].item.name is: " + this.item_array[loc].item.name);
	}
...
Inventory_Rectangle.prototype.update_rule_3 = function(item){
	...
	if(tmp_box != null){
		...
		//pw.print("chosen one is: "+ tmp_box.name);

		//add item to box
		tmp_box.add_item(item);

		//then re-draw that section of the inventory so that it now includes the item.
		var tmp_ctx = this.inventory_canvas.getContext("2d");
		tmp_ctx = tmp_box.draw_on_inventory_rect(tmp_ctx);
	}
	else{
		console.log("ERROR! ERROR WILL ROBINSON! you should not have gotten here.");
	}
...
Inventory_Rectangle.prototype.set_inventory_size = function(size){
	...
	//for (var i = 0; i < 2; i++){
		...
		var y_offset_from_rest_of_screen = this.y;
		...
		tmp_box.y = down_this_num_of_boxes + num_of_buffers + y_offset_from_rest_of_screen;

		// console.log("name: " + tmp_box.name);
		// console.log("this.y is: " + this.y);
		// console.log("tmp_box.y is: " + tmp_box.y);
		// console.log("combined is: " + (this.y + tmp_box.y));
...
Inventory_Rectangle.prototype.contains = function(item){
	...
	if(this.percentage_filled_by_item(item)){
		pw.print("returning true");
		return true;
	}
	else{
		pw.print("returning FALSE");
		return false;
	}
...
Inventory_Rectangle.prototype.percentage_filled_by_item = function(item){
	...
	if(!ulc && !llc && urc && !lrc){
		...
		//pw.print("percentage is: " + ((width * height) / total));
