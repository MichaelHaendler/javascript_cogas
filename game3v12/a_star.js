...
//moving_thing: is the "thing" (aka a person, a vechicle, etc)
//that is going to be moving from point A to point B.

//world: is the 2d (maybe eventually 3d) array of the world 

//dest: array that contains the x, y (and maybe eventually z) coordinates
//of where the character needs to go. something like 

//dest is the mx and my that was clicked on. 

/*
to do: 

a character needs to define their literal feet. will use that to define
where they are standing (and set that location in the world as a place
where others cannot stand). 

dest is pretty easy. that'll be some rounding. 

and terrain_holder will just be th. 
*/

function create_path(moving_thing,terrain_holder,dest){

	var world = terrain_holder.get_ascii_tss();

	var pathStart = moving_thing.get_current_loc();

	var pathEnd = terrain_holder.get_coords_of_containing_tb(dest);

	var currentPath = [];


	if (world[pathStart[0]][pathStart[1]] != 0 || world[pathEnd[0]][pathEnd[1]] != 0){
		break;
	}
	else{
